...
1. Size an adaptive square grid inside the study circlew
...
3. Let counts = PoIs per cell; nonempty = counts greater than 0
    1. AUI_occupany = share of cells with 1 or more PoIs
...
3. For each sample node, count PoIs reachable within network radius AUI_cell_L_m/2 using Dijkstra’s algo on length
...
5. Compute
    ...
    2. cpp_top10_share = sum of loads at or above the 90th percentile / total land
...
## see BIBLIOGRAPHY.md for full references
## see CHANGES.md for full version history/evolution (and thought processes!)
